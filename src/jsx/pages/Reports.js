 ...
 * 
 * This Reports component handles both Gold_Togo users and regular users:
 * 
 * GOLD_TOGO USERS (user.access === 'Gold-Togo' or 'Gold_Togo'):
 * - Uses Togo Reports code (from Minexx-Togo)
 * - Shows Gold mineral data
 * - Restricted to Togo country
 * - Dashboard access: 'gold'
 * - Menu: TogoMenu
 * 
 * REGULAR USERS (non-Gold_Togo):
 * - Uses standard Minexx Reports code
 * - Shows: Cassiterite, Coltan, Wolframite, Copper
 * - No Gold mineral data
 * - Dashboard access: '3ts'
 * - Menu: Based on user.type (investor, buyer, etc.)
 * 
 * KEY LOGIC:
 * - isGoldTogoUser: Determines if user has Gold_Togo access
 * - getChartSeriesTrend(): Conditionally includes Gold series
 * - chartSeries_Purchase: Shows Gold only for Gold_Togo users
 */
...
const Reports = ({ language, country }) => {
    ...
    // Helper – flatten key-value style reports into row arrays
    const flatRows = (pairs) => pairs.map(([label, value]) => ({ Metric: label, Value: value }));
    // ────────────────────────────────────────────────────────────────────
    const getExportTableRows = () => trace.exports.map((exp) => ({
        [t("Date")]: exp.date ?? '',
        [t("MineralType")]: exp.mineral ?? '',
        [t("Grade")]: exp.grade ?? '',
        [t("NetWeightKg")]: access === '3ts' ? (exp.netWeight ?? '') : ((Number(exp.netWeight) || 0) / 1000).toFixed(2),
        [t("GrossWeightKg")]: access === '3ts' ? (exp.grossWeight ?? '') : ((Number(exp.grossWeight) || 0) / 1000).toFixed(2),
        [t("ExportationID")]: exp.exportationID ?? '',
        [t("RMBRepresentative")]: exp.rmbRep ?? '',
        [t("ExporterRepresentative")]: exp.exportRep ?? '',
        [t("TraceabilityAgent")]: exp.traceabilityAgent ?? '',
        [t("Destination")]: exp.destination ?? '',
        [t("Itinerary")]: exp.itinerary ?? '',
        [t("ShipmentNumber")]: exp.shipmentNumber ?? '',
        [t("ExportCertificateNumber")]: exp.exportCert ?? '',
        [t("RRACertificateNumber")]: exp.rraCert ?? '',
        [t("ExportValueUSD")]: exp.value ?? '',
        [t("Transporter")]: exp.transporter ?? '',
        [t("IDNumberOfDriver")]: exp.driverID ?? '',
        [t("TruckPlateNumberFront")]: exp.truckFrontPlate ?? '',
        [t("TruckPlateNumberBack")]: exp.truckBackPlate ?? '',
        [t("NumberOfTags")]: exp.tags ?? '',
        [t("TotalGrossWeightKg")]: exp.totalGrossWeight ?? '',
        [t("TotalNetWeightKg")]: exp.totalNetWeight ?? '',
        [t("Attachments")]: exp.id ? `/exports/${exp.id}` : '',
    }));

    const getSupplierCompositionTableRows = () => {
        const data = rangeapplied ? (appliedChemicalComposition || []) : (chemicalComposition || []);

        return data.map((composition) => ({
            [t("Supplier")]: composition.SupplierName || '-',
            'Lot Number': composition.LotNumber || '-',
            [t("SN %")]: composition.sn_percent || '-',
            [t("Moisture %")]: composition.moisture_percent || '-',
            [t("U3O8 %")]: composition.u3o8_percent || '-',
            [t("ThO2 %")]: composition.tho2_percent || '-',
            [t("SiO2 %")]: composition.sio2_percent || '-',
            [t("MgO %")]: composition.mgo_percent || '-',
            [t("P2O5 %")]: composition.p2o5_percent || '-',
            [t("SO3 %")]: composition.so3_percent || '-',
            [t("CL %")]: composition.cl_percent || '-',
            [t("K2O %")]: composition.k2o_percent || '-',
            [t("TIO2 %")]: composition.tio2_percent || '-',
            [t("Cr2O3 %")]: composition.cr2o3_percent || '-',
            [t("MnO %")]: composition.mno_percent || '-',
            [t("Fe2O3 %")]: composition.fe2o3_percent || '-',
            [t("CuO %")]: composition.cuo_percent || '-',
            [t("ZnO %")]: composition.zno_percent || composition.zn_percent || '-',
            [t("ZrO2 %")]: composition.zro2_percent || '-',
            [t("Nb2O5 %")]: composition.nb2o5_percent || '-',
            [t("Ta2O5 %")]: composition.ta2o5_percent || composition.ta_percent || '-',
            [t("Al2O3 %")]: composition.al2o3_percent || composition.ai2o3_percent || '-',
            [t("Na2O %")]: composition.na2o_percent || '-',
            [t("CaO %")]: composition.cao_percent || '-',
            [t("Rb2O %")]: composition.rb2o_percent || '-',
            [t("SrO %")]: composition.sro_percent || '-',
            [t("Y2O3 %")]: composition.y2o3_percent || '-',
            [t("CeO2 %")]: composition.ceo2_percent || '-',
            [t("SnO2 %")]: composition.sno2_percent || '-',
            [t("I %")]: composition.i_percent || '-',
            [t("PbO %")]: composition.pbo_percent || '-',
            [t("HfO2 %")]: composition.hfo2_percent || composition.hso2_percent || '-',
            [t("BaO %")]: composition.bao_percent || '-',
            [t("As %")]: composition.as_percent || '-',
            [t("Bi %")]: composition.bi_percent || '-',
            [t("Pb %")]: composition.ph_percent || '-',
            [t("Fe %")]: composition.fe_percent || '-',
            [t("WO3 %")]: composition.wo_percent || composition.wo3_percent || '-',
            [t("Sb %")]: composition.sb_percent || '-',
            [t("Cu %")]: composition.cu_percent || '-',
            [t("Ag %")]: composition.ag_percent || '-',
            [t("Co %")]: composition.co_percent || '-',
            [t("Ni %")]: composition.ni_percent || '-',
            [t("Mn %")]: composition.mn_percent || '-',
            [t("Bal %")]: composition.bal_percent || '-',
            [t("Ti %")]: composition.ti_percent || '-',
        }));
    };
    ...
    const loadSuppliersforGrade = () => {
        ...
        axiosInstance.get(`/report/suppliertrends`, {
            params: {
                country: normalizedCountry,
            }
        })
    ...
    const chartSeries_Purchase = (isGoldTogoUser && isGoldAccess)
        ? [{
            name: 'Gold',
    ...
    const getChartSeriesTrend = () => {
        ...
        // Show Gold only for Gold_Togo users
        if (isGoldTogoUser) {
            if (access === 'gold') {
                return [goldSeries];
            } else {
                                                    ...
                                                    <Tab.Container defaultActiveKey="production">
                                                        ...
                                                        <Tab.Content className='mt-10' style={{ marginTop: 25 }}>
                                                            <Tab.Pane eventKey="production" id='production'>
                                                                <div className='card'>
                                                                    <div className='card-header d-flex justify-content-between align-items-center'>
                                                                        <h4 className='card-title mb-0'>{t("Production")}</h4>
                                                                        <button
                                                                            className='btn btn-success btn-sm'
                                                                            onClick={() => {
                                                                                const rows = access === '3ts'
                                                                                    ? trace.production.map(p => ({ weight: p.weight, location: p.location, rmbRep: p.rmbRep, traceAgent: p.traceAgent, operator: p.operator, bags: p.bags, totalWeight: p.totalWeight, note: p.note }))
                                                                                    : (trace.production?.production || []).map(row => Array.isArray(row) ? Object.fromEntries(row.map((v, i) => [i, v])) : row);
                                                                                const keys = rows.length > 0 ? Object.keys(rows[0]) : [];
                                                                                exportToCSV(rows, keys, 'trace_production');
                                                                            }}
                                                                        >
                                                            ...
                                                            {access === `3ts` ?
                                                                <>
                                                                    <Tab.Pane eventKey="bags" id='bags'>
                                                                        <div className='card'>
                                                                            <div className='card-header d-flex justify-content-between align-items-center'>
                                                                                <h4 className='card-title mb-0'>{t("BagsProduced")}</h4>
                                                                                <button
                                                                                    className='btn btn-success btn-sm'
                                                                                    onClick={() => exportToCSV(
                                                                                        trace.bags.map(b => ({ tag: b.tag, weight: b.weight, tunnel: b.tunnel, date: b.date, miner: b.miner, transporter: b.transporter, rmbRep: b.rmbRep, security: b.security, concentrate: b.concentrate, color: b.color, transport: b.transport, itinerary: b.itinerary, time: b.time, production: b.production })),
                                                                                        ['tag', 'weight', 'tunnel', 'date', 'miner', 'transporter', 'rmbRep', 'security', 'concentrate', 'color', 'transport', 'itinerary', 'time', 'production'],
                                                                                        'trace_bags'
                                                                                    )}
                                                                                >
                                                                                    <i className='fa fa-download me-1'></i> Export CSV
                                                                    ...
                                                                    <Tab.Pane eventKey="processing" id='processing'>
                                                                        <div className='card'>
                                                                            <div className='card-header d-flex justify-content-between align-items-center'>
                                                                                <h4 className='card-title mb-0'>{t("Processing")}</h4>
                                                                                <button
                                                                                    className='btn btn-success btn-sm'
                                                                                    onClick={() => {
                                                                                        const rows = trace.processing || [];
                                                                                        const keys = rows.length > 0 ? Object.keys(rows[0]) : [];
                                                                                        exportToCSV(rows, keys, 'trace_processing');
                                                                                    }}
                                                                                >
                                                                                    <i className='fa fa-download me-1'></i> Export CSV
                                                                                </button>
                                                                    ...
                                                                    <Tab.Pane eventKey="drums" id='drums'>
                                                                        <div className='card'>
                                                                            <div className='card-header d-flex justify-content-between align-items-center'>
                                                                                <h4 className='card-title mb-0'>{t("Drums")}</h4>
                                                                                <button
                                                                                    className='btn btn-success btn-sm'
                                                                                    onClick={() => exportToCSV(
                                                                                        trace.drums.map(d => ({ drum: d.drum, grossWeight: d.grossWeight, netWeight: d.netWeight, itsci: d.itsci, color: d.color, grade: d.grade, blending: d.blending, asi: d.asi })),
                                                                                        ['drum', 'grossWeight', 'netWeight', 'itsci', 'color', 'grade', 'blending', 'asi'],
                                                                                        'trace_drums'
                                                                                    )}
                                                                                >
                                                                                    <i className='fa fa-download me-1'></i> Export CSV
                                                            ...
                                                            <Tab.Pane eventKey="exports" id='exports'>
                                                                <div className='card'>
                                                                    <div className='card-header d-flex justify-content-between align-items-center'>
                                                                        <h4 className='card-title mb-0'>{t("Exports")}</h4>
                                                                        <button
                                                                            className='btn btn-success btn-sm'
                                                                            onClick={() => {
                                                                                const rows = getExportTableRows();
                                                                                const headers = rows.length > 0 ? Object.keys(rows[0]) : [];
                                                                                exportToCSV(rows, headers, 'trace_exports');
                                                                            }}
                                                                        >
                                                                            <i className='fa fa-download me-1'></i> Export CSV
                                                                        </button>
                                            ...
                                            type === 'sale' ? (
                                                ...
                                                <div className='row'>
                                                    ...
                                                    {mineral && (
                                                        <div className='card'>
                                                            <div className='card-header d-flex justify-content-between align-items-center'>
                                                                <h4 className='card-title mb-0'>{mineral} {t("CassiteriteSaleReport")}</h4>
                                                                <button
                                                                    className='btn btn-success btn-sm'
                                                                    onClick={() => exportToCSV(
                                                            ...
                                                            <div className='card'>
                                                                <div className='card-header d-flex justify-content-between align-items-center'>
                                                                    ...
                                                                    <button
                                                                        className='btn btn-success btn-sm'
                                                                        onClick={() => {
                                                                            const rows = getSupplierCompositionTableRows();
                                                                            const headers = rows.length > 0 ? Object.keys(rows[0]) : [];
                                                                            exportToCSV(rows, headers, `${mineral}_chemical_composition`);
                                                                        }}
                                                                    >
                                                                        <i className='fa fa-download me-1'></i> Export CSV
                                                                    </button>
                                                    ...
                                                    type === 'shipped' ? (
                                                        ...
                                                        <div className='row'>
                                                            ...
                                                            {mineral && (
                                                                <div className='card'>
                                                                    <div className='card-header d-flex justify-content-between align-items-center'>
                                                                        <h4 className='card-title mb-0'>{mineral} {t("CassiteriteShippedReport")}</h4>
                                                                        <button
                                                                            className='btn btn-success btn-sm'
                                                                            onClick={() => exportToCSV(
                                                                                shippedData.map(r => ({ Volume_Kg: r.Volume, Month: getMonthName(r.Month), Year: r.Year })),
                                                                                ['Volume_Kg', 'Month', 'Year'],
                                                                                `${mineral}_shipped_report`
                                                                            )}
                                                                        >
                                                                            <i className='fa fa-download me-1'></i> Export CSV
                                                                        </button>
